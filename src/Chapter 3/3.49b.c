...

{
    ...
    int flag, tmp, UserCode, Busercode; /* code inserted by user */

    int FDig, SDig, TDig, FrDig;

    printf("\nInsert four digit code to dencrypt : "); scanf("%d", &UserCode);

    Busercode = UserCode;

     FrDig = (( Busercode / 1 ) % 10 + 3) % 10; /* quarta cifra */

     TDig = (( Busercode / 10 ) % 10 + 3) % 10; /* terza cifra */

     SDig = (( Busercode / 100 ) % 10 + 3) % 10; /* seconda cifra */

     FDig = (( Busercode / 1000 ) % 10 + 3) % 10; /* prima cifra */
    ...
    /* swap second digit with fourth digit */
     ...
     /* follow instruction generate an decrypted four digit integer */
