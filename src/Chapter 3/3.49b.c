...

{
    ...
    int UserCode; /* code inserted by user */

    int tmp, FDig, SDig, TDig, FrDig;

    printf("\nInsert four digit code to decrypt : "); scanf("%d", &UserCode);

     FrDig = (( UserCode / 1 ) % 10 + 3) % 10; /* fourth digit */

     TDig = (( UserCode / 10 ) % 10 + 3) % 10; /* third digit  */

     SDig = (( UserCode / 100 ) % 10 + 3) % 10; /* second digit */

     FDig = (( UserCode / 1000 ) % 10 + 3) % 10; /* first digit */
    ...
    /* swap second digit with fourth digit */
     ...
     FrDig = tmp;
     /* follow instruction generate a decrypted four digit integer */
     UserCode = 0;
